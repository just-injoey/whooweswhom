...
def joinus(request):
    return render(request,'joinus2.html',{})
...
def room_list(request):
    ...
    return render(request,'room_list.html',{'rooms_list':rooms_list,'length':length})


"""
I have added a new view
"""
def room_listNewRoom(request):
    return render(request,'room_listNewRoom.html',{})
...
"""
I again created a new view for adding transaction
"""
def room_detailTransaction(request,pk):
    # pk = pk
    # rooms = get_object_or_404(room)
    # rooms = get_object_or_404(room)
    # rooms = room.pk
    
    # return render(request,'room_detailTransaction.html',{})
    rooms = get_object_or_404(room,pk=pk)
    creator = False
    if request.user == rooms.creater:
        creator = True
    ...
    return render(request,'room_detailTransaction.html',{'rooms':rooms,
                                                        'members_list':members_list,
                                                        'transactions':transactions,
                                                        'members_count':members_count,
                                                        'creator':creator})
...
def transaction_details(request,pk):
    ...
    return render(request,'transaction_details.html',{'transaction':transactions,'transaction_splitters_username':transaction_splitters_username,'all_room_members':all_room_members,'transaction_splitters':transaction_splitters})
...
def my_debts(request):
    ...
    return render(request,'my_debts.html',{'incomes':income,'expenses':expense})
...
def final_settlements(request):
    ...
    if len(user_sender_positive) == 0 and len(user_receiver_negative) == 0:
        noexpense = True
    return render(request,'final_settlements.html',{'user_sender_positive':user_sender_positive,
...
# my_debtsSettleDebts
def my_debtsSettleDebts(request,pk):
    income = debt.objects.filter(pk=pk)
    expense = debt.objects.filter(pk=pk)
    # income = debt.objects.filter(receiver=request.user)
    # expense = debt.objects.filter(sender=request.user)
    query = request.GET.get("q", None)
    if query is not None:
    ...
    return render(request,'my_debtsSettleDebts.html',{'incomes':income,'expenses':expense, 'pkey':pk})
    # return render(request,'my_debtsSettleDebts.html',{'usname':usname,'t_reason':t_reason, 'pkey':pk})
...
def personal_budget(request):
    ...
    template = 'personal_budget.html'
    # template = 'PersonalBudgetNew.html'
    return render(request, template, context)
...
def delete_personal_income(request,pk):
    if request.method == "POST":
        ...
        obj.delete()
        return HttpResponseRedirect(reverse('splitter:personal_budget'))
    return render(request,'personal_budget.html',{'obj':obj})
...
def delete_personal_expense(request,pk):
    if request.method == "POST":
        ...
        object.delete()
        return HttpResponseRedirect(reverse('splitter:personal_budget'))
    return render(request,'personal_budget.html',{'object':object})
